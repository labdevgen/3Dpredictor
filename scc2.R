...
processed_loop <- prep1(H1_loop, H2_loop, binsize, h, maxdist)
processed_half <- prep1_half_smoof(H1, H2, binsize, h, maxdist)

j = get.scc1(processed, binsize, maxdist)
j_loop = get.scc1(processed_loop, binsize, maxdist)
...
write.table(paste("scc = ",as.numeric(j$scc)," scc_half_smoof = ",as.numeric(j_half$scc)," scc_loop_only = ",as.numeric(j_loop$scc)," corr_loop_only = ",corr_loop, sep=" "),out_fname)
